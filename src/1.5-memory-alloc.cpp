...
int main(){
    ...
    // 输出device prt向设备要一个100 byte的线性内存, 并返回地址
    ...
    float new_value = 555;
    checkDriver(cuMemsetD32((CUdeviceptr)host_page_locked_memory, *(int*)&new_value, 1)); // cuMemsetD32：用于将指定的32位值（即四个字节）填充到GPU内存的指定区域。
